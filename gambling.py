...
class gambling(commands.Cog):
    ...
    async def blackjacking(self, ctx, bet):
        author = ctx.author
    ...
    async def blackjack(self, ctx, bet=None):
        ...
        for task in asyncio.all_tasks():
            name = str(task.get_name())
            if name == f'blackjack {author.id}':
                await ctx.send("You already in a game of blackjack.")
                return
        asyncio.create_task(self.blackjacking(ctx, bet), name=f"blackjack {author.id}")
